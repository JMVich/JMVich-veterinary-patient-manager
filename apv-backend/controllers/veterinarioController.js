...
const comprobarToken = async (req, res) => {
    ...
    if (tokenValido) {
        //Token válido, usuario si existe
        res.json({ msg: 'Token válido, usuario si existe' })
    } else {
        const error = new Error("Token no válido")
        return res.status(400).json({ msg: error.message });
    }
...
const actualizarPassword = async (req, res) => {
    ...
    //Comprobar su password (comparamos pwd actual con el que hay en la base de datos)
